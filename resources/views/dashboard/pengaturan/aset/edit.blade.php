            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        @endphp
                        <form action="{{ route('pengaturan-update-aset', $catId) }}" class="mt-4" method="POST">
                            @method('PUT')
                            @csrf
                            <div class="row mb-3">
                                <div class="col">
                            ...
                            <div class="table-responsive mb-3">
                                <table class="table table-bordered" id="subticket-table">
                                    ...
                                    <tbody id="subticket-list">
                                        @if ($category->assetSubcategories->count() > 0)
                                            @foreach ($category->assetSubcategories as $item)
                                                @php
                                                    $ascId = Crypt::encrypt($item->id);
                                                @endphp
                                                </thead>
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input type="hidden" class="form-control"
                                                            value="{{ $item->name }}, {{ $ascId }}" name="sub[]">
                                                    </td>
...
@section('script')
    <script>
        function subFor(id) {
            ...
            const fdasc = document.querySelector('#fdasc');
            fdasc.setAttribute('action', "/aset-subcategory/delete/" + id);
            fdasc.submit();
        }
    </script>
@endsection
